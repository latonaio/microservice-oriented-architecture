...
## AIONにおけるマイクロサービスメッシュアーキテクチャ   
AIONでは、Kubernetes PodとService Brokerを活用したマイクロサービスメッシュアーキテクチャが実現されています。  
また、AIONでは標準化されたマイクロサービスの定義ファイルが設定されているため、複雑なアプリケーション環境においても、ソースコードの開発、デプロイ、メンテナンスを簡単に行うことができます。
